...
<xsl:transform xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template name="validate-issue-date">
		...
		<!-- KDUBL-R-002: IssueDate is mandatory -->
		<xsl:choose>
			<xsl:when test="$context/cbc:IssueDate and normalize-space($context/cbc:IssueDate) != ''"/>
			<xsl:otherwise>
				<svrl:failed-assert test="cbc:IssueDate and normalize-space(cbc:IssueDate) != ''" flag="fatal" id="KDUBL-R-002">
					<xsl:attribute name="location">
						<xsl:apply-templates select="$context" mode="schematron-select-full-path"/>
					</xsl:attribute>
					<svrl:text>[KDUBL-R-002] Invoice issue date (cbc:IssueDate) is mandatory.</svrl:text>
				</svrl:failed-assert>
			</xsl:otherwise>
		</xsl:choose>

		<!-- KDUBL-R-002b: IssueDate format must be YYYY-MM-DD -->
	...
	<xsl:template name="validate-issue-time">
		...
		<!-- KDUBL-R-002a: IssueTime is mandatory -->
		<xsl:choose>
			<xsl:when test="$context/cbc:IssueTime and normalize-space($context/cbc:IssueTime) != ''"/>
			<xsl:otherwise>
				<svrl:failed-assert test="cbc:IssueTime and normalize-space(cbc:IssueTime) != ''" flag="fatal" id="KDUBL-R-002a">
					<xsl:attribute name="location">
						<xsl:apply-templates select="$context" mode="schematron-select-full-path"/>
					</xsl:attribute>
					<svrl:text>[KDUBL-R-002a] Invoice issue time (cbc:IssueTime) is mandatory.</svrl:text>
				</svrl:failed-assert>
			</xsl:otherwise>
		</xsl:choose>

		<!-- KDUBL-R-002c: IssueTime format must be HH:MM:SS or HH:MM:SS+/-TZ:TZ -->
